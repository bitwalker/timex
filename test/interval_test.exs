defmodule IntervalTests do
  ...
  test "can get duration of an interval" do
    duration = Interval.new(from: Date.from({2014, 9, 22}), until: [months: 5])
               |> Interval.duration(:months)
    assert 5 == duration

    duration = Interval.new(from: Date.from({{2014, 9, 22}, {15, 30, 0}}), until: [mins: 20])
