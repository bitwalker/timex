defmodule TimezoneTests do
  ...
  test :convert do
    ...
    # If it's noon in GMT+2, then it's 5'oclock in the evening in GMT-3
    assert DateTime[hour: 17] = DateTime[year: 2014, month: 2, day: 24, hour: 12, timezone: gmt_plus_two] |> Timezone.convert(gmt_minus_three)
    # If it's noon in GMT-3, then it's 5'oclock in the morning in GMT+2
    assert DateTime[hour: 7] = DateTime[year: 2014, month: 2, day: 24, hour: 12, timezone: gmt_minus_three] |> Timezone.convert(gmt_plus_two)
  end
end